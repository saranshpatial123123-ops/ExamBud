...
const Waves: React.FC<WavesProps> = ({
  ...
  useEffect(() => {
    ...
    function movePoints(time: number) {
      const lines = linesRef.current, noise = noiseRef.current, mouse = mouseRef.current;
      ...
      lines.forEach(pts => {
        pts.forEach((p: any) => {
          ...
          // Mouse Repulsion Force (Dynamic Interaction)
          ...
          if (distSq < radiusSq) {
            ...
            const force = (1 - (dist / 250)) * 40; // Max 40px push
            p.wave.x += (dx / dist) * force;
            p.wave.y += (dy / dist) * force;
          }
        });
      });
    }
  ...
  return (
    <div
      ref={containerRef}
      className={`fixed inset-0 -z-10 waves ${className} opacity-15 pointer-events-none`.trim()}
      style={{
        margin: 0, padding: 0,
    ...
    >
      <canvas ref={canvasRef} className="waves-canvas w-full h-full" />
    </div>
  );
};
